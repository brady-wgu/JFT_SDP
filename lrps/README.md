...
2. They locate the appropriate provider row (e.g., "SkillProof Tenant Admin Portal — Course Configuration")
...
This LRPS landing models step 2. The four live SkillProof rows (Student, Tenant Admin, Instructor, Super Admin) are clickable and deep-link straight into the corresponding portal HTML in this storyboard.

## What's in the table

| Type | Count | Behavior |
|:-----|:-----:|:---------|
| **Live SkillProof rows** | 4 | Clickable → launch the corresponding portal (Student, Tenant Admin, Instructor, Super Admin) |
...
The table also has a sidebar quick-launch section that links directly to the three SkillProof admin portals, and a meta-bar at the bottom with quick-launch chips to all 5 surfaces + the catalog.

## LMS → LRPS → LTI provisioning chain (student-facing flow)

LRPS is what produces the **student-facing** Coding Coach link as well. The provisioning chain that gets Sally into SkillProof from her zyBooks course page works like this:

1. **WGU's D&D (Design & Development) team** registers the SkillProof Coding Coach as an LRPS provider — a row in the LRPS provider table similar to the SkillProof-TA / SkillProof-IN / SkillProof-SA admin rows shown in this storyboard, but configured as a student-facing resource (no MFA, learner role mapping).
...
4. **LRPS issues an LTI 1.3 launch** to SkillProof. The LTI payload carries Sally's WGU student ID, course context (E010), and role (`Learner`).
5. **SkillProof receives the launch**, looks up or creates Sally's session against her WGU student ID, and opens the Coding Coach welcome screen.
...
- **Basic LTI 1.3 for the deployed MVP; LTI Advantage post-MVP.** The student MVP that JFT deployed uses basic LTI 1.3 with a stable launch URL — no grade passback, no NRPS, no per-sub-section deep linking content selection. That keeps FERPA scope minimal and the integration surface small for the first release. **LTI Advantage services are in scope for post-MVP per Appendix A §16.3 #8.1** ("Full LTI 1.3 (including Advantage services)"), and they are what enable the per-sub-section deep linking described in the Student and Tenant Admin user profile documents (Sally launching directly into a specific competency sub-section; Alice creating and managing deep link URLs that target specific platform sub-sections). The basic-vs-Advantage distinction is a phase boundary, not a contractual exclusion.
...
The LRPS landing was originally rendered in the legacy enterprise Bootstrap aesthetic (matching the actual internal tool). It was **restyled with the SkillProof Design System v1.2** in commit `e735361` so it reads as part of the SkillProof product family. Notable patterns:

- Standard SkillProof navbar (navy + WGU FY26 corporate logo + dark mode toggle + product chip + user avatar)
- Sidebar nav as an SkillProof card (Learning Resources / Search / Quick Launch · Live)
...
- Live SkillProof rows tinted Ice Blue with a 4px brand-blue left edge and a "Launch ↗" link in the actions column
- SkillProof-styled pagination footer with `.btn-tertiary` icon buttons and form-control pager input
...
- Container max-width is `1920px` to use the full widescreen viewport — the real LRPS is a data-dense admin tool, and the SkillProof's default 1192px container made the table too cramped (see commit `8704b3b` for the layout fix).
